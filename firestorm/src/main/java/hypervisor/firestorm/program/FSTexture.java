...
public class FSTexture{
    ...
    public void activateUnit(){
        GLES32.glActiveTexture(GLES32.GL_TEXTURE0 + texunit.value);
    }

    public void bind(){
        GLES32.glBindTexture(target.value, id);
    }

    public void unbind(){
        GLES32.glBindTexture(target.value, 0);
    }

    public void storage2D(int levels, int internalformat, int width, int height){
        GLES32.glTexStorage2D(target.value, levels, internalformat, width, height);
    }

    public void image2D(int level, Bitmap bitmap){
        GLUtils.texImage2D(target.value, level, bitmap, 0);
        bitmap.recycle();
    }

    public void image2D(int level, int internalformat, int width, int height, int border, int format, int type, Buffer pixels){
        GLES32.glTexImage2D(target.value, level, internalformat, width, height, border, format, type, pixels);
    }

    public void subImage2D(int level, int xoffset, int yoffset, Bitmap bitmap){
        GLUtils.texSubImage2D(target.value, level, xoffset, yoffset, bitmap);
    }

    public void subImage2D(int level, int xoffset, int yoffset, int width, int height, int format, int type, Buffer pixels){
        GLES32.glTexSubImage2D(target.value, level, xoffset, yoffset, width, height, format, type, pixels);
    }

    public void storage3D(int levels, int internalFormat, int width, int height, int depth){
        GLES32.glTexStorage3D(target.value, levels, internalFormat, width, height, depth);
    }
    ...
    public void image3D(int level, int internalformat, int width, int height, int depth, int border, int format, int type, Buffer pixels){
        GLES32.glTexImage3D(target.value, level, internalformat, width, height, depth, border, format, type, pixels);
    }
    ...
    public void image3D(int level, int internalformat, int width, int height, int depth, int border, int format, int type, int offset){
        GLES32.glTexImage3D(target.value, level, internalformat, width, height, depth, border, format, type, offset);
    }
    ...
    public void subImage3D(int level, int xoffset, int yoffset, int zoffset, int width, int height, int depth, int format, int type, Buffer pixels){
        GLES32.glTexSubImage3D(target.value, level, xoffset, yoffset, zoffset, width, height, depth, format, type, pixels);
    }
    ...
    public void subImage3D(int level, int xoffset, int yoffset, int zoffset, int width, int height, int depth, int format, int type, int offset){
        GLES32.glTexSubImage3D(target.value, level, xoffset, yoffset, zoffset, width, height, depth, format, type, offset);
    }
    ...
    public void generateMipMap(){
        GLES32.glGenerateMipmap(target.value);
    }
    
    public void wrapS(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_WRAP_S, mode);
    }

    public void wrapT(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_WRAP_T, mode);
    }

    public void wrapR(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_WRAP_R, mode);
    }

    public void minFilter(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_MIN_FILTER, mode);
    }

    public void magFilter(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_MAG_FILTER, mode);
    }

    public void compareMode(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_COMPARE_MODE, mode);
    }

    public void compareFunc(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_COMPARE_FUNC, mode);
    }

    public void baseLevel(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_BASE_LEVEL, mode);
    }

    public void maxLevel(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_MAX_LEVEL, mode);
    }

    public void swizzleR(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_SWIZZLE_R, mode);
    }

    public void swizzleG(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_SWIZZLE_G, mode);
    }

    public void swizzleB(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_SWIZZLE_B, mode);
    }

    public void swizzleA(int mode){
        GLES32.glTexParameteri(target.value, GLES32.GL_TEXTURE_SWIZZLE_A, mode);
    }
