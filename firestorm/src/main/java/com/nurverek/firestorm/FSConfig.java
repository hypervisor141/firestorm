...
public abstract class FSConfig implements VLCopyable<FSConfig>, VLLoggable{
    ...
    public static final Mode MODE_FULLTIME = new Mode(){
        ...
        @Override
        public void configure(FSRPass pass, FSConfig self, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex){
            self.configure(pass, program, mesh, meshindex, passindex);
        }

        @Override
        public void configureDebug(FSRPass pass, FSConfig self, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex, VLLog log, int debug){
    ...
    public void run(FSRPass pass, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex){
        mode.configure(pass, this, program, mesh, meshindex, passindex);
    }

    public void runDebug(FSRPass pass, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex, VLLog log, int debug){
        mode.configureDebug(pass, this, program, mesh, meshindex, passindex, log, debug);
    }

    protected abstract void configure(FSRPass pass, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex);

    protected void configureDebug(FSRPass pass, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex, VLLog log, int debug){
        try{
    ...
    protected void printDebugInfo(FSRPass pass, FSP program, FSMesh<? extends FSInstance> mesh, VLLog log, int debug){
        if(log != null){
            printHeader(log);
    ...
    public interface Mode extends VLCopyable<Mode>{
        ...
        void configure(FSRPass pass, FSConfig self, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex);
        void configureDebug(FSRPass pass, FSConfig self, FSP program, FSMesh<? extends FSInstance> mesh, int meshindex, int passindex, VLLog log, int debug);
        String getModeName();
    }
}
